...
float interaction_scope_cpu(float3 p1_pos, float3 p2_pos, float3 p1_v)
{
...
void update_boids_cpu(Boid *boids)
{
    for (int gid = 0; gid < num_boids; ++gid)
    {
        ...
        for (int idx = 0; idx < num_boids; ++idx)
        {
            ...
            if (interaction_scope_cpu(pos, neighbour, vel) > interaction_radius_2)
            {
                continue;
            }
            if (idx == gid)
